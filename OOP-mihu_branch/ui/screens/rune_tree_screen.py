...
class RuneTreeScreen:
    ...
    def draw(self, rune_tree) -> None:
        ...
        # Tiêu đề
        title      = self.font_big.render("RUNE TREE", True, (255, 220, 50))
        title_rect = title.get_rect(center=(SCREEN_W // 2, 50))
        self.screen.blit(title, title_rect)

        hint = self.font_small.render("Tab / ESC - Close", True, (140, 140, 140))
        self.screen.blit(hint, (SCREEN_W - hint.get_width() - 16, 16))

        if not rune_tree.is_ready():
            # Chưa có Element nào
            msg  = self.font_small.render("No runes yet. Level up to earn runes!", True, (160, 160, 160))
        ...
        if not mods:
            msg = self.font_small.render("No modifiers yet. Level up to earn more!", True, (120, 120, 120))
    ...
    def _draw_legend(self, rune_tree) -> None:
        ...
        title = self.font_small.render("Rune Details:", True, (200, 200, 200))
