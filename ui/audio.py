...
class AudioManager:
    ...
    def __init__(self, volume: float = 0.6) -> None:
        ...
        self._channels: dict[str, pygame.mixer.Channel] = {}   # channel đang phát theo khoá, để stop() được
    ...
    def play(self, key: str, throttle_ms: int = 80) -> None:
        ...
        Luôn phát qua ĐÚNG 1 channel cố định cho mỗi khoá (không để
        `Sound.play()` tự xin channel mới mỗi lần) — nếu không, khi clip dài
        hơn nhịp gọi (VD footstep 2.4s nhưng gọi mỗi 0.32s) sẽ có NHIỀU bản
        chồng lên nhau phát trên các channel khác nhau, và stop() chỉ dừng
        được bản mới nhất — mấy bản cũ vẫn kêu tới hết, nghe như "không ngắt
        được". Dùng 1 channel riêng thì lần play() sau tự đè/cắt bản trước,
        và stop() luôn chắc chắn dừng đúng bản đang kêu."""
        ...
        if channel is None:
            channel = pygame.mixer.find_channel(True)   # force=True: luôn có channel, kể cả phải cướp
            self._channels[key] = channel
        if channel is not None:
            channel.play(snd)

    def stop(self, key: str) -> None:
        """Dừng NGAY sound đang phát theo khoá (VD tiếng bước chân khi player
        vừa đứng yên) — không chờ clip phát hết như khi để tự kết thúc."""
        if not self._enabled:
            return
        channel = self._channels.get(key)
        if channel is not None:
