...
class HUD:
    ...
    def draw(self, player, wave_info: str) -> None:
        ...
        self._draw_stats(player, xp_y + self.XP_H + 6)

        # ── Danh sách Rune của chiêu active ──────────────────────────────────
        rune_y    = xp_y + self.XP_H + 50
        ...
        if not all_runes:
            hint = self.font.render("Tab: Mo Rune Builder", True, (120, 120, 120))
        ...
        if inv_count > 0:
            notif_surf = self.font.render(
                f"[Tab] Mo Rune Builder  ({inv_count} rune cho)",
                True, (255, 220, 50))
    ...
    def _draw_stats(self, player, y: int) -> None:
        ...
        parts = []
        if player.armor > 0:
            parts.append((f"Giáp {int(player.armor)}%", (120, 180, 255)))
